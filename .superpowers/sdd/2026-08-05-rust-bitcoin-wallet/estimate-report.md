...
| [#141](https://github.com/nhitranbtc/blockchain-sdk/pull/141) | 13/14 | Multi-recipient + drain send (sub-task A of #131) | `btc wallet send --to <addr>:<amount>` (repeatable, MAX_RECIPIENTS=20) + `--drain-to <addr>` (single destination) + `--exclude-utxo` (repeatable) + `--dry-run` (signed PSBT preview). F25 sanitize (no `:{e}` interpolation); F25 PSBT-broadcast warning; mainnet `--confirm-yes yes` gate (Story 10 extended). PR #141 SHA `f515188`. | 6 |
| [#142](https://github.com/nhitranbtc/blockchain-sdk/pull/142) | 15/16 | Coin-selection + manual UTXO (sub-task B of #131) | `btc wallet send --coin-selection bnb|knapsack|oldest` (default `bnb`; bdk 3.1 has no standalone `Knapsack` impl — `LargestFirstCoinSelection` mapped) + `--input <txid:vout>` (repeatable) + `--manual-selection-only` (requires `--input`). L12 H1+H3+M2+L1 fixes applied. PR #142 SHA `b68f391`. | 5 |
| [#143](https://github.com/nhitranbtc/blockchain-sdk/pull/143) | 17 | RBF bump-fee (sub-task C of #131) | `btc wallet bump-fee --txid <id> --fee-rate <N>` (BIP-125 RBF). Lib-layer enforces `fee_rate > original_fee_rate` (RBF rule 3 → exit 4). `BuildFeeBumpError` sanitized via Debug-format (txid-only variants per inspection). PR #143 SHA `0395466`. | 3 |
| **Total v0.1.1 hours (reference)** | | | | **24.5** |
...
- **2026-08-14** — PR #134 (`Wallet::persist` + `load_persisted` lib layer — Story 12 / Issue #130 PR3 lib-only) merged via `--admin` (SHA `e522e78`). v0.1.1 first v0.1.2 step: bdk_file_store 0.22 integration. New `wallet::persist` module (`persist(&mut BdkWallet, &Path)` + `read_change_set(&Path)` + 4 lib tests); `Wallet` struct gains `db_path: Option<PathBuf>` field; new methods `Wallet::persist` + `Wallet::load_persisted` + `db_path()` getter; 14 existing call sites updated to pass `None` (in-memory mode unchanged); F14 persistence atomicity via bdk_file_store fsync; F34 word-count validation in `load_persisted`. Story 12 lib layer complete; CLI wiring (`--db-path <PATH>` flag on `btc wallet show`/`create`) deferred to a follow-up PR. Local verify gate 264/0/8. Fixed-fee total stays at $1,650.
- **2026-08-14** — PR #125 (`btc tx-list` CLI subcommand — Story 7 / Issue #120) merged via `--admin` (SHA `7fe7bc7`). v0.1.1 fourth deliverable: read-only Esplora tx history. New `chain::explorer::{tx_url, address_url}` URL builders (3 lib tests); `Wallet::txids()` getter for sync-populated tx graph; `btc tx-list` CLI subcommand (kebab-case to avoid clap subcommand conflict); `handle_tx_list` prints sorted deduped txids + block-explorer URLs (default text + `--json`). 3 CLI parse tests (accept + limit/json flags + mnemonic redaction). User Stories table updated: 17 of 17 stories playaround-able (was 13). Also: 3 follow-up issues opened (#126 L29 prep umbrella, #127 fee model audit, #128 fee estimator — fixes the audit gaps). Local verify gate 371/0/10. Fixed-fee total stays at $1,650.
- **2026-08-14** — PR #124 (`btc fee-estimates` CLI subcommand — Story 8 / Issue #121) merged via `--admin` (SHA `d466795`). v0.1.1 third deliverable: read-only Esplora fee estimator. Top-level CLI subcommand (not under `wallet`). `print_fee_table` helper renders `target_blocks | sat/vB` (sorted ascending, defensive drop on non-u64 keys); `--json` flag for machine-readable output; reuses `default_url_for` + `parse_spki_pin_hex` + `EsploraClient::from_config` (F20 SPKI pin + F36 HTTPS-only enforcement inherited). 3 CLI parse tests (accept + `--json` + reject missing network). Local verify gate 368/0/10. Fixed-fee total stays at $1,650.
- **2026-08-14** — PR #123 (`btc wallet send --fee-rate` — Story 6 / Issue #119) merged via `--admin` (SHA `b5e3074`). v0.1.1 second deliverable: `Wallet::send` signature gains explicit `fee_rate: FeeRate` param (DEFAULT constant removed from lib); CLI gains `--fee-rate <SAT_PER_VB>` flag with `>= 1` validation; Esplora-estimate default deferred to Story 8 #121. 2 new CLI parse tests (accept + debug-redaction). Local verify gate 365/0/10. Fixed-fee total stays at $1,650.
- **2026-08-14** — PR #122 (`btc wallet send` subcommand — Story 5 / Issue #118) merged via `--admin` (SHA `61fd0df`). v0.1.1 first deliverable: `tx::{builder,sign,broadcast}` lib modules + `Wallet::send` (compose) + `EsploraClient::broadcast_tx` + `btc wallet send` CLI subcommand + `btc-send-demo.sh` (4 gates: CLI flag surface + cross-network rejection + invalid-address rejection + F36 HTTPS-only). 3 lib tests + 4 CLI tests + 4 demo gates. Cross-network rejection via `Address::require_network` defends against "send to wrong chain" operator error; F36 HTTPS-only URL reuses existing `EsploraUrl::new` validator; L12 CRITICAL #2 mnemonic redaction in Debug; default fee rate 1 sat/vB (Story 6 #119 will add `--fee-rate` override). CI two cancellations on Test + btc-cli-demo jobs (cold-cache timeout on cold runner, NOT code defect — local verify gate passed 363/0/10). Regtest integration test deferred to v0.1.1 follow-up. Fixed-fee total stays at $1,650.
...
- **2026-08-15** — Issue #131 closed (`Task #131: send extensions — Stories 13-17`). Sub-tasks A + B + C all merged to parent `task/131-send-extensions` (3 PRs: #141 + #142 + #143), then parent cut to main at merge commit `2e67dac`. v0.1.1 Stories 13/14/15/16/17 all shipped. CHANGELOG progress: 19 of 19 stories playaround-able after sub-task A; 21 of 21 after sub-task B; 22 of 22 after sub-task C. Issue #131 (parent) closed; #138/#139/#140 (sub-tasks) closed. Sub-task workflow applied: parent branch + sequential merges + PR-to-parent + final cut. Fixed-fee total stays at $1,650.
