...
- **`[user-facing]`** `btc wallet sync` + `btc wallet balance` CLI subcommands (Issue #63 / Task 54c) — stateless wrappers around `Wallet::sync` + `Wallet::balance` (lib API from PR #55). Takes `--mnemonic <words> --network <NET> --esplora-url <URL> [--pin-spki <hex64>]`. `sync` prints `n_utxos=<N> total_sat=<S>` (scriptable); `balance` prints the single sats integer. No wallet persistence — mnemonic lives in process memory only (F14 explicitly deferred; each invocation re-syncs). Threat-model coverage: F12 (chain scan via Esplora `/address/{addr}/utxo`), F13 (confirmed-only UTXO aggregation, MAX_MONEY cap), F20 (SPKI pin required for non-regtest networks — regtest exempted for localhost development), F36 (https-only URL enforced via `EsploraUrl::new` per #36). Manual `Debug` redacts the `mnemonic` field (L12 CRITICAL #2). Live testnet smoke `#[ignore]` per L29; 17 unit tests cover parse + F20 + F36 + mnemonic redaction.
