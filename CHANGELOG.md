...
| 13 | [x] **Use btc CLI subcommand** | done (PR #70, Task 54d; extended by PR #61, PR #62, PR #73, PR #74) | `cargo run -p btc -- wallet create --words 12 --network testnet` then `cargo run -p btc -- wallet show <wallet_id> --network testnet`; `cargo run -p btc -- message sign --mnemonic "<12-word phrase>" --network testnet --address <ADDR> "x"`; `cargo run -p btc -- encrypt --password hunter2 --in /tmp/plain.txt --out /tmp/cipher.enc` then `btc decrypt --password hunter2 --in /tmp/cipher.enc --out /tmp/recovered.txt` |
