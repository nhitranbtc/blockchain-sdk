...
## 11. Umbrella alignment (v0.2+ migration paths)

This design ships BTC-only as a host for the umbrella, matching v0.1 scope. v0.2+ introduces ETH + SOL; this section sketches the migration paths so v1 code doesn't paint us into a corner.

### 11.1 Host-layer location (deviation from umbrella §3.1)

**Umbrella says:** "HOST LAYER (not in this repo): iOS (UniFFI) | Android (UniFFI) | CLI | test harness".

**This design lives IN the repo** at top-level `wallet-desktop/` (renamed from `flutter-btc-wallet/` to drop the BTC coupling — see §11.2). Rationale: release coupling wants one tag per version, and the umbrella's host-layer rule predates the Flutter UI direction.

**Tracking:** ADR-candidate for an umbrella spec amendment. Until then, treat the Flutter UI as a **deviation** — same role (host layer) but in-repo.

### 11.2 Naming — BTC coupling

The directory was originally `flutter-btc-wallet/` and the Flutter `pubspec.yaml` package name is `flutter_btc_wallet`. v0.2+ brings ETH + SOL, so:

- **Directory rename:** `flutter-btc-wallet/` → `wallet-desktop/`. Class names like `BtcInvoker` / `BtcLogFilter` stay BTC-specific; new `ChainInvoker` interface gets added under which `BtcInvoker` is one impl.
- **Dart package rename:** `flutter_btc_wallet` → `wallet_desktop`. Tracked as a separate task before v0.2 ships.
- **Bundled binaries:** `assets/btc/` becomes `assets/bin/` (or `assets/<chain>/<chain>-cli`). Each per-chain CLI bundles separately.

### 11.3 Migration to `ChainWallet` trait (umbrella §4)

Umbrella §4 defines `ChainWallet` trait (v0.2). v1 BTC-only design doesn't use it — `BtcInvoker` calls `btc` directly. Migration sketch:

```dart
// v0.2+ UI abstraction (sketch only — not implemented in v1)
abstract class ChainInvoker {
  Future<T> invoke<T>(ChainCommand cmd, {required T Function(dynamic) parse});
}

class BtcInvoker implements ChainInvoker { ... }   // wraps btc CLI
class EthInvoker implements ChainInvoker { ... }   // wraps eth CLI (v0.2)
class SolInvoker implements ChainInvoker { ... }   // wraps sol CLI (v0.2)
```

DTOs currently named `WalletInfo`, `WalletDetail`, `SendResult`, etc. become generic over umbrella's associated types:

```dart
class WalletInfo<Addr, Amt, Txid> { ... }
```

This is a non-trivial migration; deferred to a v0.2 spec. v1 ships BTC-shaped DTOs.

### 11.4 Shared mnemonic across chains

Umbrella §3.2: "Mnemonic file is shared (read-only after unlock)" across per-chain crates. v1 UI keeps one mnemonic per wallet (BTC). v0.2 UI introduces `MnemonicSession`:

```dart
class MnemonicSession {            // v0.2+
  final ZeroizingString mnemonic;  // one seed → many coins
  final List<WalletSession> children;  // one per chain derived
}
```

`walletSessionProvider` becomes a child lookup under `mnemonicSessionProvider`. UI screens gain a "chain picker" dropdown.

### 11.5 ADR 0001 v1.0 hardware-signing path

ADR 0001 says v1.0 = hardware via `Signer` trait; iOS host provides `TangemSdk` impl. The Flutter UI v1.0 does not change its secret-handling contract — still no key material in UI — but the bundled `btc` becomes a thin client over UniFFI exposed by `rust-wallet-app`. UI calls don't change shape. Tracked separately under Phase 3.

### 11.6 Planned umbrella crates that touch this design

- `rust-wallet-cli` (umbrella §3.3): clap CLI like `btc` but multi-chain. If/when it ships, this UI may need to switch from bundling `btc` to bundling `rust-wallet-cli`. Cost: rename of binary assets + re-tag DTOs.
- `chain-traits` (umbrella §4): defines `ChainWallet`. v1 UI doesn't reference it directly. v0.2+ uses it via Dart equivalents.
- `rust-wallet-server` (umbrella §3.3): REST daemon, deferred v2. UI does not use it.

---

## 12. Naming migration table (v1 → v0.2)

| v1 (this design) | v0.2 (umbrella-aligned) | Effort |
|---|---|---|
| `flutter-btc-wallet/` (project dir) | `wallet-desktop/` | done at design §11.2 |
| `pubspec.yaml` package `flutter_btc_wallet` | `wallet_desktop` | rename + import update |
| `assets/btc/<arch>/btc[.exe]` | `assets/<chain>/<arch>/<chain>-cli[.exe]` | per-chain asset layout |
| `BtcInvoker` | `ChainInvoker` interface + `BtcInvoker` impl | new interface, BTC impl unchanged |
| `BtcCommand` (sealed BTC-only) | `ChainCommand` (generic over umbrella §4) | non-trivial |
| `BtcError` (exit code → kind) | `ChainError` (one per-chain impl) | per-chain |
| `BtcLogFilter` | `ChainLogFilter` (generic over mnemonic regex) | trivial |
| DTOs in `core/btc/models/` | DTOs in `core/chain/models/` (generic) | non-trivial |
| `walletSessionProvider(walletId)` | `mnemonicSessionProvider` + `walletSessionProvider(chainId, walletId)` | non-trivial |

Each row tracked as separate v0.2 task. v1 ships without these.
