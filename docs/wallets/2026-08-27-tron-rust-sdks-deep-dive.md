...
| `tron-rs` | **Don't choose.** Three reasons: (1) no signing primitive — `k256::sign_prehash` not exposed, so value-bearing tx build impossible without wrapping another crate anyway; (2) no JSON-RPC client — TronGrid mainnet + Nile both serve `/wallet/*` JSON-RPC, this crate targets gRPC only (Cosmos roots); (3) `cosmrs` baggage — pulls Cosmos-SDK tx types (`StdSignDoc`, ADR-036) that TRON does not use. v0.1.0 on crates.io (2026-01-20), 14.88k DL, single maintainer. **Useful when** (not our case): Cosmos-flavored tooling, read-only gRPC indexer, schema bootstrap for non-Rust consumers, Cosmos↔TRON bridge scaffolding. For our wallet — schema reference only: cross-check against canonical `core/Tron.proto` (`tronprotocol/java-tron` SHA `851575d` 2026-07-14) before pinning. |
