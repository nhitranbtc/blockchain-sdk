...
pub fn secp256k1_sign(sk: &[u8], msg: &[u8]) -> Result<(Vec<u8>, u8)> {
    // ZEROIZING GAP PATCH (Risk Register #3 + plan Task 0.7): the raw `sk`
    // slice was previously held in a plain `let sk = ...` binding with no
    // hygiene. We now copy it into a `Zeroizing<[u8; 32]>` buffer so the
    // caller-side secret never lingers past function return. Caller-side
    // `Zeroizing<Vec<u8>>` wrap (already in place at `tron-wallet-core`)
    // is belt-and-suspenders: even a stack copy left by `libsecp256k1`
    // is overwritten by `Zeroizing::Drop` on scope exit.
    let mut sk_buf: Zeroizing<[u8; 32]> = Zeroizing::new([0u8; 32]);
    sk_buf.copy_from_slice(sk);
    let sk = libsecp256k1::SecretKey::parse_slice(&sk_buf[..])?;
