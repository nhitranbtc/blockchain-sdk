...
pub mod transaction {
    ...
    impl ::protobuf::Message for Raw {
        ...
        fn write_to_with_cached_sizes(&self, os: &mut ::protobuf::CodedOutputStream<'_>) -> ::protobuf::Result<()> {
            ...
            if self.fee_limit != 0 {
                // Q13 (issue #540) — REVERTED 2026-09-06 after live-broadcast
                // investigation. The plan's hypothesis (fee_limit tag 18 emits
                // a spurious 0x01 byte that TronGrid mis-parses) was wrong:
                // both the standard 6-byte form (`90 01 80 c9 fe 3d` for
                // fee_limit = 130_000_000) and the attempted 5-byte form
                // (`90 80 c9 fe 3d`) fail against TronGrid with different
                // errors (NPE on `/wallet/broadcasttransaction`, then
                // InvalidProtocolBufferException on `/wallet/broadcasthex`).
                //
                // The actual fix lives in `tx::sign::sign_tx` (new
                // `signed_envelope_hex` field) and `chain::TronGridClient::broadcast`
                // (switched to `/wallet/broadcasthex` with full Transaction
                // envelope). Fee_limit encoding here is left as standard
                // protobuf varint — round-trips cleanly through
                // `TronTransaction::from_bytes` (verified by the round-trip
                // guard in sign_tx).
