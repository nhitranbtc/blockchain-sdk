...
- [L18] `ManuallyDrop<T>` + `ptr::read` is panic-safe replacement for `mem::forget` + `ptr::read` in move-out-before-drop — no Drop-during-panic gap
- [L19] `Key::from_slice` (aes-gcm) and similar FFI/constructor calls panic on wrong-length input, not Result — always guard length before passing; convert panic → `Error::Encryption`
- [L20] `const _: () = assert!(...)` is the clippy-clean way to enforce compile-time bounds on constants
- [L21] Pipeline display must use the project 6 stages (Intent, Rebase, Review, Test, Document, Lint) per `CLAUDE.md` Task display rule — sub-activities (TDD, L12 review, L12 fixes) belong in row detail, not extra stages
...
## L18 — `ManuallyDrop<T>` + `ptr::read` is panic-safe replacement for `mem::forget` + `ptr::read`

**Trigger**: Task 5 L12 security review flagged `Secret::into_inner` (Task 1.5) for panic-safety gap:
```rust
pub fn into_inner(self) -> T {
    #[allow(unsafe_code)]
    let v = unsafe { std::ptr::read(&self.0) };
    std::mem::forget(self);  // ← panic between read and forget = double-zeroize
    v
}
```
If anything between `ptr::read` and `mem::forget` panics (e.g., a destructor on `self.0`), the value is read but the wrapper is NOT forgotten — Drop runs and double-zeroizes (or worse, double-frees non-Zeroize types).

**Rule**: For move-out-before-drop patterns, prefer `ManuallyDrop<T>` over `mem::forget`:
```rust
pub fn into_inner(self) -> T {
    let me = std::mem::ManuallyDrop::new(self);
    let v = unsafe { std::ptr::read(&me.0) };
    v  // me's Drop is suppressed; only the unsafe `read` is needed
}
```
Single `unsafe` block (the `read`). No `mem::forget`. No panic-safety gap.

**Why**: `ManuallyDrop` is the zero-cost abstraction for "I promise to manage this drop myself." The `mem::forget` pattern is older and doesn't compose as cleanly with destructors. Since Rust 1.32, `ManuallyDrop` is the canonical pattern.

**Apply**: When implementing any `into_inner` / `extract` / move-out on a struct with a `Drop` impl, prefer `ManuallyDrop<T>` over `mem::forget`. Existing F53 pattern in `keys::secret::into_inner` should be migrated to this pattern (Task 5 follow-up or Task 6 sweep).

**Preview for Task 9**: `Wallet::from_mnemonic` will need to extract the bump-balance signer from a `Signer` newtype — same pattern candidate.

---

## L19 — `Key::from_slice` (aes-gcm) and similar FFI/constructor calls panic on wrong-length input

**Trigger**: Task 5 L12 type-design CRITICAL:
```rust
let cipher = Aes256Gcm::new(Key::<Aes256Gcm>::from_slice(key.expose().as_slice()));
```
`Key::from_slice` returns `&Key<Aes256Gcm>` for any length ≤ 32 bytes. For length > 32, it PANICS (`debug_assert!` on length 32). Same pattern exists in many FFI-style crates (cbc, chacha20poly1305, etc.).

**Rule**: For any FFI / newtype constructor that takes a length parameter and doesn't return Result, ALWAYS guard the length at the API boundary before calling. Convert panic to `Error::XX` rather than letting the panic propagate to the caller.

**Why**: `Key::from_slice` has no `Result` return — the operation can fail in production but the type signature hides it. Calling code that relies on the type signature for safety is wrong. The fix is caller-side: explicit length check before the FFI call.
...
**Apply**: For every constructor / newtype-wrap call that takes a length parameter, audit the source crate for `debug_assert!` or `assert!` on the length. If the assert is load-bearing (i.e., produces a panic), wrap with a length guard at the API boundary. Especially relevant for crypto crates where wrong-length inputs are a DoS vector.

**Caveat**: Some crates document the panic behavior as "the caller must guarantee this" — but the type signature doesn't encode that guarantee. Use the type system OR explicit guards.

---

## L20 — `const _: () = assert!(...)` is the clippy-clean way to enforce compile-time bounds on constants

**Trigger**: Task 5 (after L12 review) — added defensive runtime guards:
```rust
assert!(
    ARGON2_M_COST_KIB > 0 && ARGON2_T_COST >= 1 && ARGON2_P_COST >= 1 && DERIVED_KEY_LEN >= 4,
    "argon2 parameter bounds violated at compile time"
);
```
Clippy `--all-features -D warnings` flagged this as `clippy::assertions_on_constants` (the assertion is on values that are statically known).

**Rule**: When you want to enforce that a constant is within bounds, use a module-level `const _: () = assert!(...)` block instead of an in-function `assert!`. The runtime check is also dead code per the security review (constants are vetted by hand) — the compile-time block is the only useful check.

```rust
const ARGON2_M_COST_KIB: u32 = 256 * 1024;
...
**Why**: The `const _: () = assert!(...)` block runs at compile time, fails the build if the assertion fails, and has zero runtime cost. The in-function `assert!` is dead code that clippy correctly identifies.

**Apply**: For any constant in the codebase that depends on out-of-range hand-checking (Argon2 params, network ports, magic numbers), use `const _` blocks. Eliminates clippy::assertions_on_constants and makes the bounds explicit.

---

## L21 — Pipeline display must use the project 6 stages (Intent, Rebase, Review, Test, Document, Lint)

**Trigger**: Task 5 — my pipeline presented 9 ad-hoc stages (Intent, Branch, Implementation, L12 review, L12 fixes, Verify, Commit + push + PR, PAUSE merge, Ledger + lessons, Compact). Project `CLAUDE.md` Task display rule mandates exactly 6 stages in this order unless the user overrides:
...
Sub-activities (TDD, L12 review, fix round, merge pause, ledger update) belong in the row's progress detail or as separate notes — not as additional stages.

**Rule**: Every pipeline presented in this project must show the 6 canonical stages with the canonical icons (`✓ ⋮ ○`). Map ad-hoc stages to the canonical 6 like this:

| Ad-hoc stage | Maps to |
|---|---|
| Branch from main | **Rebase** (trunk-based: branch from main, never rebase during a task) |
| TDD test-first cycle | **Test** (TDD is the test-led dev cycle, not a separate stage) |
| L12 pre-PR review | **Review** |
| L12 fix round | **Test** (re-verify the suite after fix) |
| cargo fmt / clippy / geiger | **Lint** |
| Module doc + drift table + PR body | **Document** |
| Ledger update + lessons.md | **Document** (post-merge) |
| Merge authorization pause | Approval gate — not a stage |
| Compact for new session | Session lifecycle — not a stage |

**Why**: CLAUDE.md Task display rule is explicit and mandatory. Drift costs the user-conversation context (they have to mentally re-map ad-hoc stages to the canonical 6). The rule keeps one fixed shape across all tasks in the project.

**Caveat**: The user can override the 6 stages explicitly for a specific task. Default: follow them.

**Apply**: From Task 6 (bip137) onward, every pipeline diagram shows the 6 stages. The "Document" stage is its own row with a checklist:
- Module doc on new public types
- Drift table mapping plan → implementation
- Threat-model mapping (defends / does-not-defend)
- PR body with L9 v3 schema (drift table + technical details + test gaps + migration + per-dimension verdict)
- Ledger entry update (`.superpowers/sdd/<plan>/progress.md`)
- Lessons.md update (if any new L-number emerged)

**Retroactive correction for Task 5**: my pipeline was non-conforming. The correct 6-stage mapping for Task 5 would be:

```
Pipeline
task/5-encryption                                  merged
  ✓ Intent
  ✓ Rebase                                   (branched from main)
  ✓ Review                                   (L12: 3 parallel sub-agents)
  ✓ Test                                     (TDD + 85 tests passing)
  ✓ Document                                 (module docs + drift + PR body + lessons L17/L18)
  ✓ Lint                                     (fmt + clippy -D warnings + geiger)
```
